...
void *readtx(void *arg){
  ...
  zgt_v(0);
  zgt_hlink *ht = ZGT_Ht->find(1,obno);
  if(ht == NULL){
    zgt_p(0);
    ZGT_Ht->add(tx,1,obno,'S');
    //ZGT_Ht->print_ht();
    zgt_v(0);
    tx->perform_readWrite(tid,obno,'S');
    tx->status = TR_ACTIVE;
    finish_operation(node->tid);
    pthread_exit(NULL);
  }
  else{
    tx->set_lock(tid,1,obno,count,'S');
    finish_operation(node->tid);
    pthread_exit(NULL);
  }
...
void *writetx(void *arg){ //do the operations for writing; similar to readTx
  ...
  if(ht == NULL){
    zgt_p(0);
    ZGT_Ht->add(tx,1,obno,'X');
    //ZGT_Ht->print_ht();
    zgt_v(0);
    tx->status = TR_ACTIVE;
    tx->perform_readWrite(tid,obno,'X');
    finish_operation(node->tid);
    pthread_exit(NULL);
  }
  else{
    tx->set_lock(tid,1,obno,count,'X');
    finish_operation(node->tid);
    pthread_exit(NULL);
  }
...
void *do_commit_abort(long t, char status){
  ...
  if(t!=NULL){
    ...
    cout << "Tx does not exist";
    fflush(stdout);
  }
  if(status == 'A'){
    fprintf(logfile, "%d\t\t\t\t\t\t\t\t Tx Aborted\n", t);
    fflush(logfile);
  }
  else if(status == 'E'){
    fprintf(logfile, "%d\t\t\t\t\t\t\t CommitTx\n", t);
    fflush(logfile);
  }
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  zgt_p(0);
  zgt_hlink *ht = ZGT_Ht->find(sgno1,obno1);
  zgt_v(0);
  if(ht->tid == tid1){
    tx->status = TR_ACTIVE;
    ...
    if(lockmode1 == 'X'){
      ...
      zgt_p(0);
      zgt_tx *tx1 = get_tx(ht->tid);
      zgt_v(0);
      if(ht->tid != tid1){
        if(tx1->Txtype == 'W'){
          tx->status = TR_WAIT;
          zgt_p(0);
          tx1->setTx_semno(tx1->tid,tx1->tid);
          zgt_v(0);
          zgt_p(ht->tid);
          tx1->obno = -1;
          tx1->lockmode = ' ';
          tx1->status = TR_ACTIVE;
          zgt_v(ht->tid);
        }
        else if(tx1->Txtype == 'R'){
          tx->status = TR_ACTIVE;
          tx->perform_readWrite(tid1,obno1,lockmode1);
        }
      }
